...
- **🔏 EXIF 元数据清除** — 关闭压缩时，EzImage 通过 Canvas 重绘图片后再上传，移除 GPS 位置、设备信息等隐私数据，无需额外依赖。
- **🛡️ 上传可靠性** — 网络抖动时自动重试最多 2 次（退避策略）。凭证错误提示去检查设置，网络错误提示检查连接。
...
1. 从 [GitHub Releases](https://github.com/keepwonder/ezimage-obsidian/releases) 下载 `main.js`、`styles.css` 和 `manifest.json`。
...
| 选项 | 默认值 | 说明 |
| :--- | :--- | :--- |
| **压缩图片** | `开启` | 上传前转换为 WebP，同时自动清除 EXIF |
| **保持原格式** | `gif` | 这些扩展名将跳过转码、缩放和元数据清除；动态 GIF、WebP、PNG、AVIF 和 SVG 也会根据文件内容自动识别并保留 |
| **清除 EXIF 元数据** | `开启` | 移除 GPS、设备信息等元数据。仅在关闭压缩时生效（开启压缩时 WebP 转换已自动清除 EXIF）|
...
将生成的 `main.js`、`styles.css` 和 `manifest.json` 复制到 vault 的插件目录中即可本地测试。
